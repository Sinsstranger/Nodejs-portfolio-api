!(function () {
	...
	document.addEventListener('DOMContentLoaded', async () => {
		...
		const $usuallyUsedStackContainer =
			document.querySelector('[data-tech-stack-usual="true"]') || null
		const $unUsuallyUsedStackContainer =
			document.querySelector('[data-tech-stack-usual="false"]') || null
		if ($usuallyUsedStackContainer && $unUsuallyUsedStackContainer) {
			stackTechnologies.forEach(({ imagePath, title, isUsuallyUsed }) => {
				...
				let $container = isUsuallyUsed
					? $usuallyUsedStackContainer
					: $unUsuallyUsedStackContainer
		...
		if ($loginForm) {
			$loginForm.addEventListener('submit', async (e) => {
				...
				const res = await fetch(
					location.origin + e.target.getAttribute('action'),
					{
						method: 'POST',
						headers: {
